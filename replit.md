...
- **Daily Email Summaries**:
    - **User Preferences** (`user_notification_preferences` table): Users configure daily summary delivery via Profile Settings. Settings include: `dailySummaryEnabled` (boolean toggle), `dailySummaryTime` (HH:MM delivery time), `dailySummaryDays` (JSON array of weekday numbers: 0=Sunday, 1=Monday, etc.), and `timezone` (IANA timezone like "America/New_York"). Defaults: enabled=true, time=09:00, days=weekdays (1-5), timezone=Europe/London.
    - **Email Content** (generated by `daily-summary-email.ts`): Professional HTML template with gradient header, call success rate badge, stats grid (total/successful/failed/pending calls), appointment status (confirmed/rescheduled), and upcoming appointments (next 24h). Responsive design with clear typography and branded colors. Includes link to Profile Settings for preference management.
    - **Scheduled Service** (`daily-summary-service.ts`): Background service runs every 60 seconds, checks all active users with summaries enabled. Converts current UTC time to each user's configured timezone using `date-fns-tz` (toZonedTime). Compares user's local time (HH:mm) against stored delivery time (normalized from PostgreSQL time type via substring(0,5)). Sends email when time matches AND day is in delivery days array. Service logs: "📧 Checking for daily summaries to send at HH:MM UTC" every minute, "📧 Sending to [user] (HH:MM in [timezone])" on delivery.
    - **Data Aggregation**: Fetches last 24 hours of call stats (grouped by status from `call_sessions`), appointment stats (grouped by status from `contacts.appointmentStatus`), and upcoming appointments (next 24h from `contacts.appointmentTime`). Handles nullable fields (appointmentType, appointmentDate) with filters and defaults.
    - **Email Delivery** (via Resend API): Sends from "VioConcierge <notifications@vioconcierge.com>" with subject "Daily Summary - [Date]". Uses RESEND_API_KEY environment variable. Proper error handling logs failures without crashing service. Response structure handled via `result.data?.id`.
    - **Timezone Handling**: Critical fix ensures users receive emails at correct local time regardless of server timezone. Service fetches user's timezone preference, converts UTC to user's local time, normalizes PostgreSQL time format, and matches against delivery preference. Prevents issue where users in different timezones would receive emails at wrong hour.
    - **API Endpoints**: `GET /api/user/notification-preferences` returns user's settings (auto-creates defaults if missing). `PUT /api/user/notification-preferences` validates and saves preferences with schema validation (dailySummaryEnabled: boolean, dailySummaryTime: HH:MM regex, dailySummaryDays: JSON string, timezone: optional string). Both use `authenticateJWT` middleware, accessible to all authenticated users.
    - **UI Component** (Profile > Daily Summary Settings): Toggle switch for enable/disable, time picker for delivery time, timezone selector (11 common timezones: London, Paris, Berlin, New York, Chicago, Denver, Los Angeles, Tokyo, Shanghai, Dubai, Sydney), weekday buttons (Mon-Sun) with visual selection state. Shows delivery summary ("7 days a week", "Weekdays only", or "X days selected"). Save button persists changes with loading state and toast feedback.
