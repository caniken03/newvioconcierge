...
- **Comprehensive CSV Import/Export (All 32 Contact Fields)**: 
  - **Import**: Supports ALL 32 contact fields from the database schema. Fields include: Core (name, phone, groups), Appointment (date, time, type, duration, status, timezone, call before hours), Business (company, owner, location, booking source), Call Management (priority, preferred method), Tracking & Analytics (call attempts, responsiveness, sentiment), Notes & Instructions. Features intelligent field mapping with auto-detection, automatic group creation, and contact-to-group assignments. Uses multer with diskStorage for reliable file uploads up to 10MB.
  - **Export**: Generates CSV files with all 32 fields matching the database schema, including group memberships (comma-separated). Includes CSV injection protection through value escaping. Splits timestamp fields into separate date/time columns for ease of editing.
  - **Template Download**: Provides blank CSV template with all 32 field headers PLUS 15 empty sample rows ready to fill in - no more one-line Excel sheets!
  - **Sample Data Guide**: In-app UI guide showing comprehensive 32-field structure organized by category (Core, Appointment, Business, Call Management, Tracking), with format requirements and privacy warning about sensitive data.
