...
- **Appointment Reschedule Detection**: Cal.com webhook integration automatically detects and tracks appointment reschedules. When a BOOKING_RESCHEDULED event is received, the system updates the appointment status to "rescheduled" (instead of staying "confirmed") and captures both old and new appointment times in the notes field with format: "Rescheduled from [old time] to [new time] (Cal.com: [uid])". The UI displays a distinct blue "🔄 Rescheduled" badge in both Contacts and Appointments pages, providing clear visual indication that an appointment has been moved. This ensures users are immediately aware of schedule changes and can track appointment modifications through the audit trail in the notes.
